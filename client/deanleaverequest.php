...
if(isset($_SESSION['user']))
	...
	if($flag==1){
		header('location:deanrequest_leave.php?err='.urlencode(" You can launch only one leave application at a time !"));
	}
	$sql = "SELECT * FROM employees WHERE UserName = '".$user."'";
	$result = $conn->query($sql);
	if($result->num_rows > 0)
		...
		while($row = $result->fetch_assoc())
			...
			echo "<form action = 'deanrequest_confirm.php' method = 'post'>";
			echo "<table>";
