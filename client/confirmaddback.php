...
if(isset($_SESSION['user']))
	{
		$inputTextValue= array( 
		 ...
		 if($companydb->phase_b_collection->count(array("user"=>$_SESSION['user']))==0){
		 	$companydb->phase_b_collection->insertOne(array("user"=>$_SESSION['user']),$inputTextValue);
			 $updateResult=$companydb->phase_b_collection->updateOne(array("user"=>$_SESSION['user']),array('$set' => array("bgdetails" => $bgd)));
		 }
		 else{
			$updateResult=$companydb->phase_b_collection->updateOne(array("user"=>$_SESSION['user']),array('$set' => array("bgdetails" => $bgd)));
		 }
		...
		$updateResult=$companydb->phase_b_collection->updateOne(array("user"=>$_SESSION['user']),array('$push' => "bgdetails" => $bgd));
		//$updateResult=$companydb->phase_b_collection->updateOne(array("user"=>$_SESSION['user']),array('$push' => array("pubdesc" => $pubdesc)));
		//printf("Inserted %d documents", $updateResult->getInsertedCount()); var_dump($updateResult->getInsertedId());
		if($companydb->phase_b_collection->count(array("user"=>$_SESSION['user']))==0){
			$updateResult=$companydb->phase_b_collection->insertOne(array("user"=>$_SESSION['user']),array('$push' => "bgdetails" => $bgd));
			$updateResult=$companydb->phase_b_collection->updateOne(array("user"=>$_SESSION['user']),array('$push' => "bgdetails" => $bgd));
			//$updateResult=$empcollection->updateOne(array("user"=>$_SESSION['user']),array('$push' => array("pubdesc" => $pubdesc)));
