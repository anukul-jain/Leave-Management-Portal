...
$client = new MongoDB\Client(
	...
	//printf("Inserted %d documents", $insertOneResult->getInsertedCount()); var_dump($insertOneResult->getInsertedId());
...
if(isset($_SESSION['user']))
	...
	echo "<th>Add Background</th>";
	echo "<th>Add Publication</th>";
	echo "<th>Add Courses</th>";
	echo"</tr>";
	echo "<br><tr>";

	echo "<td><form action = '/' method = 'post'>";
	echo "<textarea input type = 'text' name = 'bgd' class = 'textbox shadow selected' style='width: 200px; height:200px; text-align:top; ' placeholder = 'Enter details'></textarea>";
	echo "<br><br><br><button type='submit'  class = 'login-button shadow'   formaction = 'confirmaddback.php?user=".$user."'>Insert</button> </form></td>";
	
	echo "<td><form action = '/' method = 'post'><br><input type = 'text' name = 'pubname' class = 'textbox shadow selected' placeholder = 'Publication Name'>";
	echo "<br><br><textarea input type = 'text' name = 'pubdesc' class = 'textbox shadow selected' style='width: 200; height:150px; text-align:top; ' placeholder = 'Publication Details'></textarea>";
	echo "<br><br><button type='submit'  class = 'login-button shadow'   formaction = 'confirmaddpro.php?user=".$user."'>Insert</button> </form></td>";
	
	echo "<td><form action = '/' method = 'post'><br><input type = 'text' name = 'ccode' class = 'textbox shadow selected' placeholder = 'Course Code'>";
	echo "<br><br><textarea input type = 'text' name = 'cname' class = 'textbox shadow selected' style='width: 200; height:150px; text-align:top; ' placeholder = 'Course Name'></textarea>";
	echo "<br><br><button type='submit'  class = 'login-button shadow'   formaction = 'confirmaddcourse.php?user=".$user."'>Insert</button> </form></td>";
	echo "</tr>";
	...
	if($result2->num_rows > 0){
		while($row2 = $result2->fetch_assoc()){
			...
			else if($row2['Status']=='Granted'){
				$sql3 = "SELECT * FROM deanemp_leaves WHERE EmpName ='".$empname."'";
				$result3 = $conn->query($sql3);
				while($row3 = $result3->fetch_assoc()){
					if($row3['Status']=='Requested'||$row3['Status']=='Redirected'){
						$flag=1;
						break;
					}
				}
	...
	if($flag==1){
		header('location:request_leave.php?err='.urlencode(" You can launch only one leave application at a time !"));
	}
